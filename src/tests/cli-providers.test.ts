...
describe('vclaw cli', () => {
  ...
  it('supports the omx compatibility alias and prints a deprecation notice to stderr', () => {
    const cliPath = join(process.cwd(), 'dist', 'cli', 'omx.js');
    const result = spawnSync(process.execPath, [cliPath], {
      cwd: process.cwd(),
      encoding: 'utf-8',
    });
    ...
    assert.equal(result.status, 0);
    assert.match(result.stdout, /vclaw - video-first clean-room core/);
    assert.match(result.stderr, /temporary compatibility alias/);
  });
