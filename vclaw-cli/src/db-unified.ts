 ...
 *
 * Thin pass-through to the SQLite backend (`./db`). The original veo-cli
 * shipped a dual SQLite/Convex façade, but the videoclaw-v2 merge dropped
 * Convex (per MERGE_PLAN.md §A3.2 — projects-on-disk + SQLite is the source
 * of truth; cloud state is out of scope for the merged CLI).
 *
 * This file is preserved as a shim so existing callers (cli.ts, upload.ts,
 * index.ts) keep working with their current import paths. Functions remain
 * `async` to preserve the call-site shape; the underlying SQLite calls are
 * synchronous and wrapped in `Promise.resolve(...)`.
 */

import * as sqlite from "./db";

// Re-export types unchanged (Convex variants removed)
...
/**
 * Backwards-compat shim: previously returned true when CONVEX_URL was set.
 * Always false now — Convex is removed.
 */
