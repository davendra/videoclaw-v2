...
- **2026-01-23**: **Convex database integration** - Added optional Convex cloud database support for multi-device sync and collaboration. New unified database layer (`db-unified.ts`) that automatically switches between SQLite (local) and Convex (cloud) based on `CONVEX_URL` env var. Added `sync` command to migrate existing SQLite data to Convex.
  - New files: `src/db-convex.ts` (Convex client adapter), `src/db-unified.ts` (unified interface)
  - New CLI command: `bun run google.ts sync` - migrates SQLite data to Convex
  - All CLI commands now async-compatible for Convex support
  - Backward compatible - SQLite remains default when `CONVEX_URL` is not set
...
veo-cli supports two database backends for job tracking:

| Backend | Storage | Best For |
|---------|---------|----------|
| SQLite (default) | Local `veo-cli.db` file | Single device, offline use |
| Convex (optional) | Cloud database | Multi-device sync, collaboration |

### SQLite (Default)

By default, veo-cli uses a local SQLite database (`veo-cli.db`) for tracking batches and jobs. No configuration required.

### Convex (Cloud)

For multi-device sync and real-time collaboration, you can use Convex cloud database.
...
3. Migrate existing data (optional):
   ```bash
   bun run google.ts sync
   ```

**Benefits over SQLite:**
- Multi-device sync - access from any machine
- Real-time updates - see job progress instantly
- Multi-user support - collaborate without file locks
- Automatic backups - data is stored in the cloud
- Webhook support - built-in for external integrations
...
# Optional - specify email for user tracking
CONVEX_USER_EMAIL=your-email@example.com
```
